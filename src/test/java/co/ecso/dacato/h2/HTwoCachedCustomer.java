 ...
 * @since 17.09.16
 */
final class HTwoCachedCustomer implements CachedDatabaseEntity<Long> {
    ...
    HTwoCachedCustomer(final ApplicationConfig config, final Long id) {
        this.config = config;
        this.id = id;
    }
    ...
    @Override
    public CompletableFuture<HTwoCachedCustomer> save(final ColumnList columnValuesToSet) {
        return this.update(new SingleColumnUpdateQuery<>(HTwoCachedCustomer.TABLE_NAME, "UPDATE " + TABLE_NAME + " SET %s WHERE %%s = ?",
                Fields.ID, this.id, columnValuesToSet), () -> objectValid).thenApply(rowsAffected ->
                new HTwoCachedCustomer(config, id));
    }
    ...
    public CompletableFuture<DatabaseResultField<String>> firstName() {
        return this.findOne(new SingleColumnQuery<>(HTwoCachedCustomer.TABLE_NAME, QUERY, Fields.FIRST_NAME, Fields.ID, this.primaryKey()), () ->
                this.objectValid);
    }

    public CompletableFuture<DatabaseResultField<Long>> number() {
        return this.findOne(new SingleColumnQuery<>(HTwoCachedCustomer.TABLE_NAME, QUERY, Fields.NUMBER, Fields.ID, this.primaryKey()), () ->
                this.objectValid);
    }
