...
final class CassandraCachedCustomer implements CachedDatabaseEntity<Long> {
    ...
    public CompletableFuture<CassandraCachedCustomer> save(final ColumnList columnValuesToSet) {
        return this.update(new SingleColumnUpdateQuery<>(CassandraCachedCustomer.TABLE_NAME, "UPDATE " + TABLE_NAME + " SET %s WHERE %%s = ?",
    ...
    public CompletableFuture<DatabaseResultField<String>> firstName() {
        return this.findOne(new SingleColumnQuery<>(CassandraCachedCustomer.TABLE_NAME, QUERY, Fields.FIRST_NAME, Fields.ID, this.primaryKey()), () ->
                this.objectValid);
    }

    public CompletableFuture<DatabaseResultField<Long>> number() {
        return this.findOne(new SingleColumnQuery<>(CassandraCachedCustomer.TABLE_NAME, QUERY, Fields.NUMBER, Fields.ID, this.primaryKey()), () ->
                this.objectValid);
    }
