...
final class HSQLCachedCustomer implements CachedDatabaseEntity<Long> {
    ...
    public CompletableFuture<HSQLCachedCustomer> save(final ColumnList columnValuesToSet) {
        return this.update(new SingleColumnUpdateQuery<>(HSQLCachedCustomer.TABLE_NAME, "UPDATE " + TABLE_NAME + " SET %s WHERE %%s = ?",
    ...
    public CompletableFuture<DatabaseResultField<String>> firstName() {
        return this.findOne(new SingleColumnQuery<>(HSQLCachedCustomer.TABLE_NAME, QUERY, Fields.FIRST_NAME, Fields.ID, this.primaryKey()), () ->
                this.objectValid);
    }

    public CompletableFuture<DatabaseResultField<Long>> number() {
        return this.findOne(new SingleColumnQuery<>(HSQLCachedCustomer.TABLE_NAME, QUERY, Fields.NUMBER, Fields.ID, this.primaryKey()), () ->
                this.objectValid);
    }
